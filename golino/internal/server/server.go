...
func Run(ctx context.Context) error {
    ...
    // list_cache tool
    mcp.AddTool(server, &mcp.Tool{Name: "list_cache", Description: "List cached posts from the Colino DB"}, func(ctx context.Context, req *mcp.CallToolRequest, p ListCacheParams) (*mcp.CallToolResult, any, error) {
        if p.Hours <= 0 {
            p.Hours = 24
        }
        lim := 50
    ...
    // get_content tool
    mcp.AddTool(server, &mcp.Tool{Name: "get_content", Description: "Get content by IDs, URL, or time window"}, func(ctx context.Context, req *mcp.CallToolRequest, p GetContentParams) (*mcp.CallToolResult, any, error) {
        dbPath, err := config.LoadDBPath()
        if err != nil {
            return nil, nil, err
        }
