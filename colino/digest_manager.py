...
class DigestManager:
    ...
    def digest_url(self, url: str, output_file: str = None) -> bool:
        """
    ...
    def digest_post(self, post_id: str, output_file: str = None) -> bool:
        """
        Digest content from a specific post ID
        
        Args:
            post_id: The post ID to digest
            output_file: Optional path to save digest to file
            
        Returns:
            bool: True if successful, False otherwise
        """
        logger.info(f"Processing digest for post ID: {post_id}")
        
        try:
            post = self.db.get_content_by(id=post_id)
            ...
            # Generate digest
            digest_content = self.digest_generator.summarize_article(post)
            
            # Save or display digest
            self._save_or_display_digest(digest_content, output_file, post['source'])
            return True
    ...
    def digest_youtube_video(self, youtube_video_url: str, output_file: str = None) -> bool:
        """
        Digest content from a specific YouTube video URL
        
        Args:
            youtube_video_url: The YouTube video URL to digest
            output_file: Optional path to save digest to file
            
        Returns:
            bool: True if successful, False otherwise
        """
        logger.info(f"Processing YouTube digest for: {youtube_video_url}")
        ...
        try:
            ...
            if not video_id:
                print(f"❌ Invalid YouTube URL: {youtube_video_url}")
                return False
            ...
            logger.error(f"Error generating YouTube digest: {e}")
            print(f"❌ Error generating YouTube digest: {e}")
            return False
    ...
    def _digest_cached_content(self, existing_content: Dict[str, Any], output_file: str = None) -> bool:
        """Digest content that was found in cache"""
        print(f"✅ Found cached content for {existing_content.get('url', 'unknown URL')}")
        ...
        # Save or display digest
        self._save_or_display_digest(digest_content, output_file, f"cached_{existing_content['source']}")
        return True
    
    def _digest_fresh_url(self, url: str, output_file: str = None) -> bool:
    ...
    def _digest_fresh_youtube(self, video_id: str, output_file: str = None) -> bool:
    ...
    def _digest_fresh_website(self, url: str, output_file: str = None) -> bool:
    ...
    def _save_or_display_digest(self, digest_content: str, output_file: str = None, source_type: str = ""):
