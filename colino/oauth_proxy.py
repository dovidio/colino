...
class OAuthProxyClient:
    ...
    def _initiate_auth(self) -> Tuple[str, str]:
        ...
        try:
            ...
            raise Exception(f"OAuth proxy connection failed: {e}")
        except requests.Timeout as e:
            logger.error(f"OAuth proxy server timeout: {e}")
            raise Exception(f"OAuth proxy timeout: {e}")
        except requests.HTTPError as e:
            logger.error(f"OAuth proxy HTTP error: {e}")
            raise Exception(f"OAuth proxy HTTP error: {e}")
        except (KeyError, ValueError) as e:
            logger.error(f"Invalid response from OAuth proxy: {e}")
            raise Exception(f"OAuth proxy response error: {e}")
        except requests.RequestException as e:
            logger.error(f"Failed to initiate OAuth: {e}")
            raise Exception(f"OAuth initiation failed: {e}")
        ...
        while time.time() - start_time < self.timeout:
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception("Session not found - authentication may have expired")
...
class TokenManager:
    ...
    def __init__(self, oauth_client: OAuthProxyClient, db=None, service: str = "youtube"):
        """
    ...
    def _save_tokens(self):
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"Failed to save OAuth tokens to database for {self.service}")
